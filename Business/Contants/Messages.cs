using Core.Entites.Concrete;
using System;
using System.Collections.Generic;
using System.Text;

namespace Business.Contants
{
    public static class Messages 
    {
        public static string Added = "Başarıyla eklendi";
        public static string Deleted = "Başarıyla silindi";
        public static string Error = "işlem başarısız";
        public static string InvalidName = "Geçersiz ad";
        public static string Listed = "Başarıyla listelendi";
        public static string Updated = "Güncellendi";
        public static string ColorAdded = "Renk eklendi";
        public static string ColorDeleted = "Renk silindi";
        public static string ColorUpdated = "Renk güncellendi";
        public static string ColorsListed = "Renkler listenlendi";
        public static string CustomerAdded = "Müştrei eklendi";
        public static string CustomerDeleted = "Müşteri silind";
        public static string CustomerUpdated = "Müşteri bilgisi güncellendi";
        public static string CustomersListed = "Müşteriler listelendi";
        public static string UserAdded = "Kişi eklendi";
        public static string UserDeleted = "Kişi silindi ";
        public static string UsersListed = "Kişiler listelendi";
        public static string UserUpdated = "Kişiler güncellendi";
        public static string UserNotFound = "Kullanıcı bulunamadı";
    }
}
